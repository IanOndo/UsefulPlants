...
`UsefulPlants` is an R package primarily developed as supporting software for the results and analyses conducted in the article **"The global distribution of plants used by humans"** [@UsefulPlants]. It relies upon the R package `rsdm` [@rsdm] for building species distribution models (SDMs) of plants used by humans at large scale, thus it helps to streamline global biodiversity analyses, improve code readability and reproducibility.
...
``` r
devtools::install_github("IanOndo/UsefulPlants") # optionally rebuild the vignette by setting , build_opts = c("--no-resave-data", "--no-manual")

library(UsefulPlants)
```
Please follow the vignettes from the package [**rsdm**](https://github.com/IanOndo/rsdm) for *gathering and curating plants occurrence records* and for *modelling species distributions*.  
